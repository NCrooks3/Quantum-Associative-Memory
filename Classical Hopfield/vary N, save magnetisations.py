...
# Parameters
steps = 500000       # Number of Monte Carlo steps (each step consists of N spin updates)
J = 1.0              # Coupling constant
beta = 0.0028          # Inverse temperature  
...
def monte_carlo_step(s, beta, W, n):
...
def fixed_N_trajectory(n, beta, steps):
    n = int(n)
    N = n**2
    ...
    for step in range(steps):
        ...
        if step % 20000 == 0:
            ...
            plt.savefig(f'C:\\Users\\scfro\\OneDrive\\Pictures\\mc hopfield gifs\\varying N\\bj = %s, sweep = %s, N = %s.png' % (str(beta * J), str(step), str(N)), bbox_inches='tight', pad_inches=0)
            plt.close(fig)
    ...
    filename = f'C:\\Users\\scfro\\OneDrive\\Pictures\\mc hopfield gifs\\varying N\\magnetisation_vs_time_steps_{steps}_beta_{beta}_J_{J}_N{N}.txt'
...
n_vals = np.flip(n_vals, 0)
n_vals = 40 * n_vals 
n_vals += 1

for N in n_vals:
    fixed_N_trajectory(N, beta, steps)